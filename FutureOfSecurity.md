...
* *Encrypt by default.* Cloud providers should always encrypt data in transit – via SSL, and data at rest for any data that they store. However, the challenge with encryption always has been: who holds the keys? For enterprises with strict compliance requirements, moving data to the cloud has been impossible, unless they can assure that data is persistently encrypted, and they can maintain exclusive ownership over encryption keys. This implies that customers need to have tools to encrypt data before it leaves the enterprise, while maintaining the functionality of cloud applications. New technology providing gateway encryption, and searchable strong encryption, has been adopted by many large enterprises, enabling adoption of the cloud, while maintaining compliance.
